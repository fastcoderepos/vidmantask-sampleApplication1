...
import com.fastcode.timesheet.addons.reporting.domain.report.QReportEntity;
...
public class ReportAppService implements IReportAppService {
    ...
    public void delete(Long reportId, Long userId) {
        ...
        existing.removeReportversion(
            _reportversionRepository.findById(new ReportversionId(userId, reportId, "running")).get()
        );
        existing.removeReportversion(
            _reportversionRepository.findById(new ReportversionId(userId, reportId, "published")).get()
        );
